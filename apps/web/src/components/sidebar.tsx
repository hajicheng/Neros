...
import { Archive, ArchiveRestore, BarChart3, Bot, Briefcase, ChevronDown, ChevronRight, Layers, MessageSquare, PanelLeftClose, PanelLeftOpen, Pencil, Pin, PinOff, Plus, Search, Trash2, X } from 'lucide-react'
...
export function Sidebar() {
  ...
  const setSidebarDrawerOpen = useAppStore((s) => s.setSidebarDrawerOpen)
  ...
  return (
    <>
      {/* sidebar 窄屏遮罩 —— sidebar 抽屉打开时点击关闭 */}
      {drawerOpen && (
        <div
      ...
      <aside
        className={cn(
          ...
          collapsed ? 'w-14' : 'w-72',
          // sidebar 窄屏：固定定位抽屉，默认 -translate-x-full 隐藏；打开时滑入
      ...
      {mode === 'conversations' ? (
        <>
          ...
          <div className={cn('shrink-0', collapsed ? 'flex flex-col items-center gap-2 py-2' : 'space-y-2 px-3 pt-3')}>
            {collapsed ? (
              <>
                <Button
                  size="icon"
                  variant={mainView === 'office' ? 'default' : 'outline'}
